...
def get_wassersteinized_layers_modularized(args, networks):
    '''
...
#             enumerate(zip(networks[0].named_parameters(), networks[1].named_parameters())):
    ...
    num_layers = len(list(zip(networks[0].items(), networks[1].items())))

    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in \
            enumerate(zip(networks[0].items(), networks[1].items())):
        print(f"Layer {idx}: {layer0_name} vs {layer1_name}")
        print(f"Weight 0 shape: {fc_layer0_weight.shape}, Weight 1 shape: {fc_layer1_weight.shape}")
...
def geometric_ensembling_modularized(args, local_dict_list):

    for i in range(len(local_dict_list)):
        if i > 0:
            local_dict_list[i] = get_wassersteinized_layers_modularized(args, [local_dict_list[0], local_dict_list[i]])
