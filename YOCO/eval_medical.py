...
from vqa import MedVQA
from vqa_eval import MedVQAEval
...
def report_metrics(result_file, split):
    ...
    if split == "test":
        test_list = json.load(open('../../data/medical/raw_data/vqarad_test.json'))
    ...
    with open(
        os.path.join(args.output_dir, "evaluate.txt"), "a"
    ) as f:
...
def run_inference(args):
    ...
    with open("../../data/medical/raw_data/vqarad_test.json", "r") as f:
        test_data = json.load(f)

    pred_qa_pairs = []
    gpteval_vqa_list = []
    for data in test_data:
        image_path = os.path.join(args.video_folder, data['image_name'])
    ...
    json.dump(gpteval_vqa_list, open(os.path.join(args.result_dir, "gpteval_vqa.json"), "w"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video-folder', default='../../data/medical/raw_data/images')
